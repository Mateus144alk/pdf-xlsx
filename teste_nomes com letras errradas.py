...
def normalizar_nome(nome):
    ...
    nome = ''.join(ch for ch in nome if ch not in string.punctuation)
...
# SIAPEs com nome diferente (ignorando acentuação e pontuação)
erros = []
for _, row in df_comparar.iterrows():
    ...
    if siape in siapes_certo:
        ...
        if normalizar_nome(nome) != normalizar_nome(nome_correto):
            erros.append(row)

df_erros = pd.DataFrame(erros)

# --- 💾 SALVAR RESULTADOS ---
registros_a_mais.to_excel("registros_mais.ods", index=False, engine="odf")
df_erros.to_excel("erros_nome.ods", index=False, engine="odf")

print("✔ Corrigido e arquivos gerados com sucesso:")
print(f"- SIAPEs a mais: {len(registros_a_mais)} registros")
print(f"- Nomes divergentes (ignorando acentuação/pontuação): {len(df_erros)} registros")
